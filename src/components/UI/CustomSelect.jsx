...
export default function CustomDatePicker({ 
  ...
  const [calendarMode, setCalendarMode] = useState('hijri'); // التلقائي هجري
  ...
  // حالات الاختيار المتفرقة
  const [gDay, setGDay] = useState(mainDate ? mainDate.getDate() : '');
  const [gMonth, setGMonth] = useState(mainDate ? mainDate.getMonth() : 0);
  const [gYear, setGYear] = useState(mainDate ? mainDate.getFullYear() : '');

  const [hDay, setHDay] = useState(hijriDetails ? hijriDetails.day : '');
  ...
  // خيارات الأيام مع خيار التحديد
  const dayOptions = useMemo(() => [
    { label: t('datePicker.selectDayPlaceholder', 'اختر...'), value: '' },
  ...
  ], [daysInMonth, cleanLang, t]);

  // خيارات أشهر الميلادي
  ...
  // التعامل مع التغيير في التقويم الميلادي
  const handleGregorianChange = (d, m, y) => {
    ...
    const numY = Number(y);
    const numM = Number(m);
    const numD = (!d || d === '' || isNaN(Number(d))) ? 1 : Number(d);
  ...
  // التعامل مع التغيير في التقويم الهجري
  ...
  return (
    <div className="flex flex-col w-full space-y-2 text-start" dir={isRtl ? 'rtl' : 'ltr'}>
      {/* الشريط العلوي للتبديل وحساب العمر */}
      ...
      {/* حقول الاختيار مرتبة بشكل موحد دائماً: [اليوم - الشهر - السنة] */}
      {calendarMode === 'gregorian' ? (
