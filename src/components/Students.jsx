...
export default function Students({ students, setStudents }) {
  ...
  const handleAdd = async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert([{ 
        ...
        country_code: newStudent.countryCode, // تم إضافة حفظ كود الدولة
  ...
  const sendWhatsApp = (student) => {
    ...
    const phone = student.parent_phone.replace(/^0+/, ''); // إزالة الصفر
  ...
  return (
    <div style={{ padding: "20px", color: "#fff", direction: "rtl" }}>
      <header style={{ display: "flex", justifyContent: "space-between", marginBottom: "20px" }}>
        <h2>{t("students")}</h2>
        <button onClick={() => setIsModalOpen(true)} style={{ background: C.gold, padding: "10px 20px", borderRadius: "10px", border: 'none', cursor: 'pointer' }}>+ {t("add_student")}</button>
      </header>
      ...
      {isModalOpen && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.8)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 2000 }}>
          <div style={{ background: "#1e293b", padding: "20px", borderRadius: "15px", width: "300px" }}>
            <h3 style={{ textAlign: "center" }}>{t("add_student")}</h3>
            ...
            <div style={{ display: "flex", gap: "5px", marginBottom: "10px" }}>
              ...
              <input placeholder={t("phone")} onChange={(e) => setNewStudent({...newStudent, parent_phone: e.target.value})} style={{ flex: 1, padding: "10px", borderRadius: "5px" }} />
            </div>

            <input placeholder={t("name")} onChange={(e) => setNewStudent({...newStudent, name: e.target.value})} style={{ width: "100%", padding: "10px", marginBottom: "10px", boxSizing: "border-box" }} />
            
            <button onClick={handleAdd} disabled={loading} style={{ width: "100%", padding: "12px", background: C.gold, border: 'none', borderRadius: '5px' }}>{t("save")}</button>
            <button onClick={() => setIsModalOpen(false)} style={{ width: "100%", padding: "10px", marginTop: "10px", background: "transparent", border: '1px solid #444', color: '#fff' }}>{t("cancel")}</button>
          </div>
        </div>
      )}
    </div>
