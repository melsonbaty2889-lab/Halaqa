...
import { 
  FaUserPlus, FaSearch, FaGraduationCap, FaPhone, FaBookOpen, 
  FaUserShield, FaStickyNote, FaEdit, FaTimes, FaSave, FaArchive, 
  FaEyeSlash, FaCalendarAlt, FaMoneyBillWave, FaStar
} from 'react-icons/fa';

// دالة تحويل الرقم التراكمي إلى نص مقروء ومفهوم
...
export default function Students({ students = [], setStudents, academyId }) {
  ...
  // الطالب الجاري تعديله حالياً
  const [editingStudent, setEditingStudent] = useState(null);
  const [isAdding, setIsAdding] = useState(false);
  const [updatingId, setUpdatingId] = useState(null); 
  const [message, setMessage] = useState({ text: '', type: '' });
  
  // رسائل النجاح والخطأ الموضعية بأسفل بطاقة الطالب
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{ 
          ...
          current_quarter: currentQuarterIndex, // حفظ الرقم في الحقلين للاحتياط والمطابقة
      ...
      setMessage({ text: t('student_added_success', 'تم تسجيل الطالب بنجاح! 🎉'), type: 'success' });
  ...
  // 💾 دالة تعديل وحفظ بيانات الطالب (مُصلحة بالكامل لحفظ الورد والحفظ الحالي)
  const handleUpdateStudentSubmit = async (e) => {
    e.preventDefault();
    setUpdatingId(editingStudent.id);
    setInlineMessage({ studentId: null, text: '', type: '' });
    
    // 1. حساب الفهرس المختار من الكومبوننت وإخراج النص المقابل له تلقائياً
    const selectedIndex = editingStudent.current_quarter_index || 0;
    const autoSurahText = getQuarterText(selectedIndex);

    // 2. تحديث الكائن المحلي ليحتوي على النصوص والأرقام الجديدة فوراً
    const updatedStudentData = { 
      ...editingStudent, 
      current_surah: autoSurahText,
      current_quarter: selectedIndex,
      current_quarter_index: selectedIndex
    };

    try {
      // 3. إرسال أمر التحديث لقاعدة البيانات وتحديث كافة الحقول المحتملة للحفظ والورد
      const { error } = await supabase
        ...
        .update({
          ...
          current_surah: autoSurahText,                     // تحديث النص (مثال: الجزء 1 - الربع 2)
          current_quarter: selectedIndex,                   // تحديث الرقم في العمود الأول المحتمل
          current_quarter_index: selectedIndex,             // تحديث الرقم في العمود الثاني المحتمل
      ...
      // تحديث حالة النظام واجهة المستخدم بالبيانات الجديدة مباشرة
      setStudents(prev => prev.map(st => st.id === updatedStudentData.id ? updatedStudentData : st));
      
      // إظهار رسالة النجاح في أسفل بطاقة الطالب المعدّل مباشرة
      setInlineMessage({
        studentId: updatedStudentData.id,
        text: 'تم حفظ كافة التغييرات ومزامنة مستوى حفظ الطالب بنجاح! ✅',
        type: 'success'
      });

      // إغلاق واجهة التعديل بعد رؤية رسالة النجاح
      setTimeout(() => {
      ...
      console.error("🚨 خطأ في تحديث مستوى حفظ الطالب:", error);
      setInlineMessage({
        studentId: editingStudent.id,
        text: `تعذر حفظ تعديل الحفظ: ${error.message}`,
        type: 'error'
      });
    } finally {
      setUpdatingId(null);
    ...
    <div style={{ direction: 'inherit' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        {filteredStudents.map(student => {
          ...
          // استخراج قيمة الفهرس الحالي للطالب للتأكد من ربطها بالـ Selector بشكل صحيح
          const currentStudentQuarterIndex = editingStudent ? (editingStudent.current_quarter_index ?? editingStudent.current_quarter ?? 0) : 0;
            ...
            <div key={student.id} style={{ background: C.surface, padding: '20px', borderRadius: '12px', border: isCurrentEditing ? `1px solid ${C.gold}` : `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              {isCurrentEditing ? (
                <form onSubmit={handleUpdateStudentSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  {/* 🕋 محدد الحفظ والمستوى الحالي المقترن بالدالة البرمجية المصلحة */}
                  ...
                  <div style={{ background: '#0C1520', padding: '10px', borderRadius: '6px', border: `1px solid ${C.border}` }}>
                    <QuranProgressSelector 
                      ...
                      onIndexChange={(newIndex) => setEditingStudent({ ...editingStudent, current_quarter_index: newIndex, current_quarter: newIndex })} 
                    />
                  </div>
                  ...
                  {/* صندوق الرسالة الموضعية يظهر هنا بالأسفل بالكامل فوق الأزرار مباشرة */}
                  {inlineMessage.studentId === student.id && inlineMessage.text && (
                    <div style={{ padding: '10px', borderRadius: '6px', marginTop: '5px', backgroundColor: inlineMessage.type === 'success' ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)', color: inlineMessage.type === 'success' ? '#10B981' : '#EF4444', border: `1px solid ${inlineMessage.type === 'success' ? '#10B981' : '#EF4444'}`, fontWeight: 'bold', textAlign: 'center', fontSize: '14px' }}>
                      {inlineMessage.text}
                    </div>
                  )}

                  {/* أزرار الحفظ والإلغاء الملونة بأسفل الكارد */}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    ...
                    <div style={{ marginTop: '5px', background: 'rgba(255,255,255,0.02)', padding: '10px', borderRadius: '8px', border: `1px solid ${C.border}` }}>
                      ...
                      <QuranProgressBar currentIndex={student.current_quarter_index ?? student.current_quarter ?? 0} />
                    </div>
                  </div>
