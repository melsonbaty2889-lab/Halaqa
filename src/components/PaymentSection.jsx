...
// شعارات SVG احترافية رسمية لجميع وسائل الدفع
const PaymentLogos = {
  applePay: (
    <svg width="42" height="22" viewBox="0 0 50 30" fill="none">
      ...
      <text x="18" y="20" fill="#FFF" fontSize="15" fontWeight="bold" fontFamily="sans-serif">Pay</text>
    </svg>
  ),
  mada: (
    <div style={{ background: '#00A859', color: '#FFF', padding: '2px 7px', borderRadius: '4px', fontWeight: '900', fontSize: '10px', letterSpacing: '0.5px' }}>
      mada مدى
    </div>
  ),
  instapay: (
    <div style={{ background: 'linear-gradient(135deg, #4c1d95, #6d28d9)', color: '#FFF', padding: '3px 8px', borderRadius: '5px', fontWeight: '800', fontSize: '10px', display: 'flex', alignItems: 'center', gap: '3px' }}>
      <span>⚡</span> InstaPay
    </div>
  ),
  vodafone: (
    <div style={{ background: '#e60000', color: '#FFF', padding: '3px 8px', borderRadius: '5px', fontWeight: 'bold', fontSize: '10px' }}>
      Vodafone Cash
    </div>
  ),
  fawry: (
    <div style={{ background: '#ffcc00', color: '#002b49', padding: '3px 8px', borderRadius: '5px', fontWeight: '900', fontSize: '10px' }}>
      fawry פوري
    </div>
  ),
  stcPay: (
    <div style={{ background: '#4F008C', color: '#FFF', padding: '3px 8px', borderRadius: '5px', fontWeight: 'bold', fontSize: '10px' }}>
      stc pay
    </div>
  ),
  paypal: (
    <svg width="36" height="20" viewBox="0 0 24 24" fill="none">
  ...
  card: (
    <svg width="32" height="20" viewBox="0 0 24 24" fill="none" stroke="#f8fafc" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect x="1" y="4" width="22" height="16" rx="2" ry="2"></rect>
      <line x1="1" y1="10" x2="23" y2="10"></line>
    </svg>
  ),
  bank: (
    <svg width="24" height="20" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2">
      <path d="M3 21h18M3 10h18M5 6l7-3 7 3M4 10v11M20 10v11M8 14v3M12 14v3M16 14v3"/>
    </svg>
  ),
  crypto: (
    <div style={{ background: '#26A17B', color: '#FFF', padding: '3px 6px', borderRadius: '4px', fontWeight: 'bold', fontSize: '10px' }}>
      USDT TRC20
    </div>
  )
};
...
export default function PaymentSection({ 
  ...
  // هيكلة وسائل الدفع بحسب المنطقة
  const paymentMethods = {
    ...
    gcc: [
      { id: 'apple_pay', name: 'Apple Pay', isManual: false, logo: PaymentLogos.applePay, badge: 'الأسرع' },
  ...
  const handleFormSubmit = () => {
    // إرسال البيانات بما فيها الملف المعين إن وجد
    onSubmit(currentMethodObj.id, currentMethodObj.isManual, receiptFile);
  };

  return (
    <div style={{ background: '#111827', border: '1px solid #1e293b', borderRadius: '24px', padding: '24px', maxWidth: '550px', margin: '0 auto', boxSizing: 'border-box', color: '#fff' }}>
      
      <h4 style={{ color: '#f59e0b', marginTop: '0', marginBottom: '20px', fontSize: '1.05rem', fontWeight: '700', borderBottom: '1px solid #1e293b', paddingBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <span>💳</span> {isRTL ? "اختر وسيلة الدفع المناسبة لك:" : "Select Payment Method:"}
      </h4>

      {/* قائمة وسائل الدفع بالشعارات الرسمية */}
        ...
        {activeMethods.map((item) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '45px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                {item.logo}
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              {item.badge && (
                <span style={{ background: '#f59e0b22', color: '#f59e0b', fontSize: '0.7rem', padding: '2px 6px', borderRadius: '12px', fontWeight: 'bold' }}>
                  {item.badge}
                </span>
              )}
      ...
      {/* تفاصيل التحويل اليدوي + رفع الإشعار للحماية */}
      {currentMethodObj.isManual && (
        <div style={{ marginTop: '20px', background: '#0a0f1d', border: '1px dashed #f59e0b', borderRadius: '14px', padding: '16px' }}>
          ...
          {/* حقل إرفاق صوره إشعار التحويل لحماية المتجر وقانونية العملية */}
          <div style={{ marginTop: '12px' }}>
            <label style={{ display: 'block', color: '#94a3b8', fontSize: '0.78rem', marginBottom: '6px' }}>
              📎 {isRTL ? "إرفاق صورة إشعار التحويل (مستحسن للتحقق الفوري):" : "Attach Transfer Receipt (Recommended for instant verification):"}
            </label>
            <input 
              type="file" 
              accept="image/*,.pdf"
              onChange={(e) => setReceiptFile(e.target.files[0])}
              style={{ width: '100%', padding: '6px', borderRadius: '6px', border: '1px solid #334155', background: '#111827', color: '#cbd5e1', fontSize: '0.8rem' }}
            />
          </div>
        </div>
      )}

      {/* تنبيه الدفع الآلي الفوري */}
      {!currentMethodObj.isManual && (
