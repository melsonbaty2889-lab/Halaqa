        ...
        {activeTab === 'financial' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <textarea value={student.notes || ""} placeholder={isArabic ? 'اكتب هنا تفاصيل أداء الطالب، التوصيات الصحية، أو الملاحظات الخاصة بالمشرف الأكاديمي...' : 'Enter custom administrative or health reports...'} onChange={(e) => setStudent({...student, notes: e.target.value})} disabled={!isEditing} style={{ padding: '10px 12px', borderRadius: '8px', border: '1px solid #1e293b', background: '#0C1520', color: '#fff', height: '80px', resize: 'none', fontSize: '13px', boxSizing: 'border-box', opacity: !isEditing ? 0.85 : 1, lineHeight: '1.5' }} />
            </div>
          </div>
        )}
