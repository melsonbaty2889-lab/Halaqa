  ...
  return (
    <div className={styles.dashboardContainer} style={{ paddingBottom: '80px', direction: isRtl ? 'rtl' : 'ltr', textAlign: isRtl ? 'right' : 'left' }}>
      ...
      <div style={{ background: '#1E293B', padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.08)', marginBottom: '20px' }}>
        ...
        {/* 4️⃣ قسم حلقات اليوم المباشرة أو التنبيهات العاجلة (النظام الذكي الهجين) */}
{stats.activeHalaqasData && stats.activeHalaqasData.length > 0 ? (
  /* الحالة 1: عرض الحلقات القائمة عند وجودها اليوم */
  <div style={{ background: '#1E293B', padding: '16px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.08)' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
      <h3 style={{ fontSize: '0.9rem', color: '#FFFFFF', margin: 0, fontWeight: '700', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <FaMosque style={{ color: '#38BDF8' }} />
        <span>{isArabic ? 'حلقات اليوم المباشرة' : 'Today Active Halaqas'}</span>
      </h3>
      <span style={{ fontSize: '0.75rem', color: '#94A3B8' }}>
        {stats.activeHalaqasData.length} {isArabic ? 'حلقة' : 'halaqas'}
      </span>
    </div>
  ...
  /* الحالة 2: تنبيه عاجل ينشأ فقط إذا كانت هناك متأخرات مالية بدون حلقات */
  <div style={{ background: '#1E293B', padding: '14px 16px', borderRadius: '14px', border: '1px solid rgba(248, 113, 113, 0.3)' }}>
    <div 
      onClick={() => setActiveTab && setActiveTab('payments')}
...
) : null /* الحالة 3: لا شيء يُعرض على الشاشة مطلقاً لتوفير المساحة وإلغاء التكرار */}
