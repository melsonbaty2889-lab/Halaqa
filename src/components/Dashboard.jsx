...
// 🌟 دالة مساعدة خارج الكائن لتفادي تعارض الـ Runtime والانهيار
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      try {
        ...
        -- 2. جلب عدد الطلاب التابعين للأكاديمية بدقة متناهية
        const { count: studentCount, error: studentError } = await supabase
          .from('students')
          .select('*', { count: 'exact', head: true })
          .eq('academy_id', staffData.academy_id); // زيادة تأكيد أمني للـ Multi-Tenancy
    ...
    <div style={styles.container}>
      ...
      <header style={{ ...styles.header, borderBottom: `1px solid ${C.border}` }}>
        <div>
          ...
          <p style={styles.roleBadge}>صلاحية الحساب الحالية: <span style={{color: C.gold}}>{profile?.role === 'owner' || profile?.role === 'admin' ? 'مدير الأكاديمية' : 'معلم الحلقة'}</span></p>
        </div>
      ...
      <div style={styles.grid}>
        <div style={{ ...styles.card, backgroundColor: C.surface, border: `1px solid ${C.border}` }}>
          ...
          <p style={{ ...styles.cardValue, color: C.gold }}>{formatArabicNumber(stats.studentsCount)}</p>
        ...
        <div style={{ ...styles.card, backgroundColor: C.surface, border: `1px solid ${C.border}` }}>
          ...
          <p style={{ ...styles.cardValue, color: C.text }}>{formatArabicNumber(stats.staffCount)}</p>
...
const styles = {
  container: { maxWidth: '1200px', margin: '0 auto', direction: 'rtl', fontFamily: "'Cairo', sans-serif", padding: '10px' },
