...
export default function NotificationMenu({
  ...
  return (
    <div className="relative">
      ...
      {showMenu && (
        <div 
          className={`absolute top-full mt-2 w-[calc(100vw-32px)] max-w-xs sm:w-80 border border-[var(--border-input)] rounded-2xl shadow-2xl z-50 p-3 text-xs ${
            activeRtl ? 'left-0' : 'right-0'
          }`}
          style={{ backgroundColor: 'var(--surface-card)', opacity: 1 }}
        >
          <div className="flex justify-between items-center pb-2.5 mb-2 border-b border-[var(--border-card)]">
            <div className="flex items-center gap-2">
              <span className="font-extrabold text-[var(--text-main)] text-xs sm:text-sm">
          ...
          <div className="flex items-center gap-1.5 mb-2.5 bg-[var(--surface-input)] p-1 rounded-xl border border-[var(--border-input)]">
            <button
              type="button"
              onClick={() => setNotifFilter('all')}
              className={`flex-1 py-1 text-[10px] sm:text-[11px] font-bold rounded-lg transition-all ${
                notifFilter === 'all' 
          ...
          {loadingNotifs ? (
            <div className="py-6 text-[var(--text-sub)] text-center font-medium">
              ...
              {filteredNotifications.map((item) => (
                ...
                >
                  <div className="flex flex-col gap-0.5 min-w-0">
                    <span className={`text-[11px] leading-snug ${!item.is_read ? 'font-bold text-[var(--text-main)]' : 'font-medium'}`}>
                      {item.title}
                    </span>
                    {item.message && (
                      <span className="text-[10px] text-[var(--text-sub)] line-clamp-2 leading-relaxed">
                        {item.message}
                      </span>
                    )}
                    <span className="text-[9px] text-[var(--text-sub)] opacity-75 font-mono mt-0.5">
