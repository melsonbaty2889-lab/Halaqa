...
// 🛡️ 1️⃣ استيراد المكونات ديناميكياً وعزلها تماماً لمنع انهيار بداية التشغيل
...
// 🛑 رادار الأعطال الذكي لالتقاط انهيارات الواجهات الداخلية وعرض التقرير فوراً
class AppErrorBoundary extends Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error("Internal Component Crash:", error, errorInfo);
  }
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ padding: '30px', background: '#111827', color: '#EF4444', minHeight: '100vh', fontFamily: 'monospace', direction: 'ltr', textAlign: 'left' }}>
          <h2 style={{ color: '#C9A84C', marginBottom: '10px' }}>🚨 رادار الأعطال: تم رصد مشكلة في مكون داخلي!</h2>
          <p style={{ color: '#9CA3AF' }}>التطبيق تعطل بسبب هذا الخطأ الصريح داخل الملفات:</p>
...
function AppContent() {
  // إدارة اللغة برمجياً بدون الاعتماد على استيراد i18next الثابت للحماية من السواد
  const [currentLang, setCurrentLang] = useState(() => {
    if (typeof window !== 'undefined') {
      const urlParams = new URLSearchParams(window.location.search);
      return urlParams.get('lang') || localStorage.getItem('i18nextLng') || 'ar';
    }
    return 'ar';
  });
  ...
  // 🔌 جلب ملف السوبابيس ديناميكياً وعزله لحمايته من الانهيار البدئي
  useEffect(() => {
    import('./lib/supabase')
      .then((mod) => {
        if (mod && mod.supabase) {
  ...
  // مراقبة الجلسة والتشغيل الأولي وحذف لودر الـ HTML الثابت برمجياً
  useEffect(() => {
    if (!supabase) return;
  ...
  // 🚨 عرض خطأ السوبابيس إذا كان هو السبب الرئيسي
  if (supabaseError) {
    return (
      <div style={{ padding: '30px', background: '#111827', color: '#EF4444', minHeight: '100vh', fontFamily: 'monospace', direction: 'rtl', textAlign: 'right' }}>
        <h2 style={{ color: '#C9A84C' }}>🚨 عطل في الاتصال السحابي (Supabase Setup Error)</h2>
        <p style={{ color: '#9CA3AF' }}>فشل التطبيق في قراءة متغيرات البيئة للاتصال بقاعدة البيانات على فيرسل:</p>
  ...
  // شاشة الانتظار الآمنة أثناء التحميل البدئي للملفات
    ...
    return (
      <div style={{ backgroundColor: "#090F17", minHeight: "100vh", display: "flex", justifyContent: "center", alignItems: "center", color: "#C9A84C", fontFamily: "sans-serif", direction: "rtl" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <span style={{ fontSize: "13px", color: "#4b5966" }}>جاري موازنة الحماية والملفات السحابية...</span>
  ...
  // الجدار المالي للاشتراكات المنتهية
  if (session) {
...
// 📦 3️⃣ التصدير النهائي المغلف بأعلى درجات الحماية ضد التجمد والسواد
